...
<head>
   ...
   <title>Διαγραφή στην σχέση users</title>
...
<h1>Διαγραφή στην σχέση users</h1> 
  	...
  	$a4 = $_GET['gender'];
   	...
   	if($a1 && $a2 && $a3 && $a4) {  
      	$con = pg_connect("host=$host dbname=$db user=$user password=$pass")
      	  ...
      	  $sqlp1="SELECT id FROM users WHERE name='".$a1."' AND surname='".$a2."' AND dob='".$a3."' AND gender='".$a4."'";
		  $rsap = pg_query($con, $sqlp1)  or die("Cannot execute query1: $sqlp1\n");
          $row = pg_fetch_row($rsap);
          $sqlp2 = "DELETE FROM users WHERE id=$row[0];";
