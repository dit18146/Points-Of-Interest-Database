...
$result = pg_exec($link, "SELECT city.name AS tab1 FROM city, users, residing WHERE user_id=users.id AND city_id=city.id GROUP BY city.name ORDER BY COUNT(user_id) DESC LIMIT 1;")
